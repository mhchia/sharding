...
print("!@# viper_rlp_decode_tx.addr={}".format(utils.checksum_encode(viper_rlp_decoder_addr)))
print("!@# viper_rlp_decode_tx.sender_addr={}".format(utils.checksum_encode(viper_rlp_decoder_tx.sender)))
...
def get_valmgr_ct():
    ...
    if not _valmgr_ct:
        _valmgr_ct = abi.ContractTranslator(get_valmgr_abi())
    return _valmgr_ct
...
def create_valmgr_tx(gasprice=GASPRICE):
    ...
    tx = Transaction(0, gasprice, 3000000, to=b'', value=0, data=bytecode)
    tx.v = 27
