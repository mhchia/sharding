...
def get_vmc_func_abi(func_name, contract_abi):
...
def mk_contract_tx_obj(
    ...
    func_abi = get_vmc_func_abi(func_name, contract_abi)
    arg_types = [
        arg_abi['type']
        for arg_abi
        in func_abi['inputs']
...
class BaseChainHandler:
    ...
    def deploy_initiating_contracts(self, privkey):
        if not self.is_vmc_deployed():
            ...
            for tx in txs[3:]:
                self.direct_tx(tx)
            self.mine(1)
            print('!@# deploy: vmc: ', self.et.get_transaction_receipt(eth_utils.encode_hex(txs[-1].hash)))
    ...
    def deploy_valcode_and_deposit(self, validator_index):
        ...
        self.deposit(valcode_addr, address, privkey.to_bytes())
...
class TesterChainHandler(BaseChainHandler):
    ...
    def direct_tx(self, tx):
        ...
        # evm_tx.nonce = tx.nonce
        # evm_tx.gas_price = tx.gas_price
        # evm_tx.gas = tx.gas
        # evm_tx.to = tx.to
        # evm_tx.value = tx.value
        # evm_tx.data = tx.data
        # evm_tx.v = tx.v
        # evm_tx.r = tx.r
        # evm_tx.s = tx.s
    ...
    def sample(self, shard_id):
        '''sample(shard_id: num) -> address
        '''
        tx_obj = mk_vmc_call_tx_obj('sample', [shard_id])
        print(tx_obj)
        return self.et.call(tx_obj)
        # print("!@# sample().call(): ", self.et.call(tx_obj))
        # tx_hash = self.et.send_transaction(tx_obj)
        # print("!@# sample(): tx_hash={}".format(tx_hash))
        # self.mine(1)
        # tx_receipt = self.et.get_transaction_receipt(tx_hash)
        # print("!@# sample(): get_transaction_receipt={}".format(tx_receipt))
        # return tx_hash

    def deposit(self, validation_code_addr, return_addr, privkey, startgas=TX_GAS):
        '''deposit(validation_code_addr: address, return_addr: address) -> num
        '''
        tx_obj = mk_vmc_call_tx_obj(
            'deposit',
    ...
    def withdraw(self,
        ...
        '''withdraw(validator_index: num, sig: bytes <= 1000) -> bool
        '''
        result = self._vmc.withdraw(validator_index, sig, sender=privkey, startgas=startgas)
    ...
    def add_header(self, header, privkey, startgas=TX_GAS):
        '''add_header(header: bytes <= 4096) -> bool
        '''
        self._vmc.add_header(header, sender=privkey, startgas=startgas)
    ...
    def get_num_validators(self):
        ...
        # tx_obj = mk_vmc_call_tx_obj('get_num_validators', [])
        sender_addr = keys[0].public_key.to_checksum_address()
        ...
        result = self.et.send_transaction(tx_obj)
        self.mine(1)
        print("!@# get_num_validators(): receipt", self.et.get_transaction_receipt(result))
        print('!@# get_num_validators():', result)
        return result
...
class RPCHandler(BaseChainHandler):
    ...
    def sample(self, shard_id):
        '''sample(shard_id: num) -> address
        '''
        return self._vmc.call().sample(shard_id)g
    ...
    def withdraw(self,
        ...
        address = eth_utils.address.to_checksum_address(utils.privtoaddr(privkey))
        self.unlock_account(address)
        self._vmc.transact({'from': address, 'gas': startgas}).withdraw(validator_index, sig)
    ...
    def add_header(self, header, privkey, startgas=TX_GAS):
        ...
        address = eth_utils.address.to_checksum_address(utils.privtoaddr(privkey))
        self.unlock_account(address)
        self._vmc.transact({'from': address, 'gas': startgas}).add_header(header)
    ...
    def tx_to_shard(self,
        ...
        self._vmc.transact({'from': address, 'gas': startgas, 'value': value}).tx_to_shard(
            to,
            shard_id,
            tx_startgas,
            tx_gasprice,
...
def test_handler(HandlerClass):
    ...
    print("!@# get_nonce(vmc_sender_addr)=", handler.get_nonce(handler._vmc_sender_addr))
